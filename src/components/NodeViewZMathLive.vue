<!-- 
  ...
  参考：
    ...
    Webページでは数式を表すためにLaTeX表記が使えるMathJaxを利用しています（assets 出力には使えそう）
...
<template>
  ...
  >
    ...
    <span v-if = "isDevModeRef && (!isPreviewModeRef)" class="vue3-view-label" contenteditable="false"> Vue3 MatjLive View Component</span>
    ...
    >
      ...
      <!-- 
        https://github.com/Shimada666/VaTex    
          ex. :expression="'\\frac{a_i}{1+x}'" 
      -->
      <vue-latex v-if='props.node.attrs["miniEditorCompiled"] != ""'
        ref = "presentElem"
        class = "katex" 
        :expression = 'props.node.attrs["miniEditorCompiled"]' 
        display-mode
      />
...
watch(fmini_stat_ref, (val, oldVal) => {
  ...
  if (val === 'finish') {
    props.updateAttributes({ miniEditorLangSrc: floatMinieditorRef.value.src });
    props.updateAttributes({ miniEditorCompiled: fmini_content_ref.value })
