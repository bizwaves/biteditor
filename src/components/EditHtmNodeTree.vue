<template>
  <div 
    ...
    title = "galaxy_ref :- EditHtmNodeTree galaxy: div, Position, Size の基準（当 Html Frame の 辺界線）" 
  >
    ...
    <div 
      ...
      title = "earth_ref :- EditHtmNodeTree earth: div, canvas, to warp svg-root"
    >  
      ...
      <v-container
        ...
        title = "menu_container_ref :- EditHtmlNodeTree Menu Container"
      >
        <div v-if="false"
          ref="menu_area_ref" 
      ...
      <div v-if="true" class = "meta-content-container">
        ...
        <!-- BEGIN コンテンツ実体 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
        <div class = "paired_tag_outer html_events">
...
import EditHtmTry from "./EditHtmTry.vue"
...
const edit_tiptap_ref = inject('edit_tiptap_ref');
...
//=== BEGIN mii_content_ref ※ Debug の時　↓ を切り替え出来ます

//= Mathematics
//    https://tiptap.dev/api/extensions/mathematics
//=
// const mii_content_ref = ref(`
//         <h1>
...
// text color
// const mii_content_ref = ref(`<p><span style="color: #958DF1">Oh, for some reason that’s purple.</span></p>`);
...
//= <z-math> デモ: MathLive - Math 入力
// const mii_content_ref = ref('<p> before z-math </p>  <z-math data-type="my-z-math-node"  miniEditorLangSrc=""  miniEditorCompiled=""  ></z-math>  <p> after z-math </p>');

//= <z-diag> デモ: mermaid - Diagramming and charting tool
const mii_content_ref = ref('<p> before z-diag </p>  <z-diag data-type="my-z-diag-node"  miniEditorLangSrc=""  miniEditorCompiled=""  ></z-diag>  <p> after z-diag </p>');

//= <z-code> デモ: codemirror
// const mii_content_ref = ref('<p> before z-codemirror </p>  <z-code data-type="my-z-code-node" miniEditorLangSrc="const foo = 0" miniEditorCompiled="const foo = 0;"  ></z-code>  <p> after z-codemirror </p>');

//= <z-html> デモ: 研究用?
// const mii_content_ref = ref('<p> before z-html </p>  <z-html count="0"> <p> SLOT OF Z-HTML </p> </z-html>  <p> after z-html </p>');

//=　次期 <z-katex  > The fastest math typesetting renders library for the web
//=　次期 <z-mathjax> Beautiful and accessible math renders in all browsers

//=== END mii_content_ref


const tiptap_content_ref = mii_content_ref;
// const tiptap_content_ref = ref('<p>A Vue.js wrapper component for tiptap to use <code>v-model</code>.</p>');
...
const htmsvg_inner_ref = tiptap_content_ref;   // alais
...
watch(htmsvg_inner_ref, (newVal) => { // 2024.01.08s
  console.log(`htmsvg_inner_ref changed to ${newVal}`);
  htmsvg_outer_ref.value.draw = newVal;
  console.log(`htmsvg_outer_ref.value.draw changed to ${htmsvg_outer_ref.value.draw}`);
}, { deep: true });
...
const updateLPanelAttrs = () => { // 2024.01.06c L-attr に反映
  ...
  try {
    edit_tiptap_ref.value = structuredClone(htmsvg_outer_ref.value); // 2024.01.06c L-attr に反映
...
const editor_div_ref = ref(null);  // 2023.11.20

//= OTHER := 子 TiptapEditor
const other_tag_ref     = ref(null);      // 拡張性を提供する TiptapEditor
const other_uuid        = uuidv4();       // 拡張性を提供する TiptapEditor
...
watch(c, (val, oldVal) => { // SvgEditor resizing_frame サイズが反映されたと確認出来た 2023/06/14
  console.log("$$$$EditHtmNodeTree watch galaxy c val: " + val.w + " - " + + val.h + " oldVal: " + oldVal.w + " - " + + oldVal.h);
...
console.log("@@@@@@@@ EditHtmNodeTree props: " + JSON.stringify(props, null, 2));
...
const cmenuItems = ref([
  { id: 'who_am_i',             visible: true, name: 'EditHtmlNodeTree : ' + uuidRef.value + ', deletedRef = ' + deletedRef.value },  // who am i

  { id: 'off_able_pointer',     visible: false, name: 'Disable Resize'                      },
  { id: 'on_able_pointer',      visible: false, name: 'Enable Resize'                       },

  //= 必要と判明したら、投入して下さい
  { id: 'drop_off_ctrl',        visible: false, name: 'Drop Off'                            },  // 制御権限をSubへ譲る ※ sub 側の操作で返さない限り、cmenu 表示出来ない 
  { id: 'takeup_ctrl',          visible: false, name: 'Takeup'                              },  // 制御権限を取り上げる ※ cmenu 表示されないため、動作することがない
...
watch(actingEditEventRef, (val) => {
  ...
  if ((val.cate == 'attr') && (val.event == props.com_type)) {  // 2024.01.04c TODO EditSvg*.vue しか実装していない ⇒ EditHtml*.vue にも対応すること 
    htmsvg_outer_ref.value = val.val;
    console.log('  htmsvg_outer_ref updated to ' + JSON.stringify(htmsvg_outer_ref.value, null, 2));
    return;
  }
...
watch(edit_tiptap_ref, (newVal) => { // 2023.12.19a
  if (!((actingEditRef.value.com_type == props.com_type) && (actingEditRef.value.uuid == uuidRef.value))) {  // 2024.01.06b
  ...
  console.log(`$$$$ watch edit_tiptap_ref: htmsvg_outer_ref.value updated to ${JSON.stringify(htmsvg_outer_ref.value, null, 2)}`);
}, { deep: true });
...
const saveDbContent = (mod_uuid, self_only = false) => {    // 2023.12.10a + mod_uuid
  ...
  console.log("saveContent " + JSON.stringify(item, null, 2));
...
async function sql_load_content(item, self_only = false) {
  ...
  console.log("  recordset lenth: " + recordset.length);
...
async function modCustomTagContent(tiptap: String, mode: String) {
  console.log("modCustomTagContent");
  ...
  let result = {}
  try {
    result = await window.unipyapi.makeTiptapContent(sendData);
    console.log("  result: " + JSON.stringify(result, null, 2));
  } catch (error) {
    console.error(error);
  }
  ...
  if (!("html" in result['readed_data'])) {
    console.log("  'recordset' not in result['readed_data']");
    return out_dc;
  }


  const base64Str = result['readed_data']['html']
  const normalStr = Base64.decode(base64Str)
  console.log("  modContent: " + normalStr);

  out_dc["modContent"] = normalStr;
  out_dc["hasLaTex"] = result['readed_data']['hasLaTex']
...
const makeRawContentJson = async (rootContainerRequired) => {
  ...
  //= <z-diag> 子 本コンテンツを収集する
  //    1. Q: TiptapEditor v-model := tiptap_content_ref  ⇒ z-diag のソースコードが見える
  //            ※ mii_content_ref.value は tiptap_content_ref と同じ内容である
  //       A: 下記のように処理すると良い
  ...
  //                ※ &quot ⇒ "
  //      参照： C:\DOCs\SvgEditor\mod\ 元 exam01-assets.html, svg 表示可能 mod-exam01-assets.html
  //      参考： https://zenn.dev/dozo/articles/a727ef8a9212f2
  ...
  // -------------------------------------------------------------------------------
  // <p data-uid="956f6f64-414f-4e2b-808b-cf34630014c1">before z-mermaid</p>
  // <z-diag minieditorlangsrc="
  //   sequenceDiagram
  ...
  //   quot; y2=&quot;273&quot; x2=&quot;75&quot; y1=&quot;273&quot; x1=&quot;332&quot;>
  //   </line>
  //   </svg>
  //   " 
  //   data-type="my-z-diag-node"
  // >
  //   <div>
  //   sequenceDiagram
  //       Alice-&gt;&gt;+John: Hello John, how are you?
  ...
  //   </div>
  // </z-diag>
  // <p data-uid="8c51aa84-19b7-4d41-adec-6280ce13b68c">after z-mermaid</p></div> 
  // -------------------------------------------------------------------------------
  //= 

  //= <z-math> 子 本コンテンツを収集する
  //    1. Q: TiptapEditor v-model := tiptap_content_ref  ⇒ z-math のソースコードが見える
  //            ※ mii_content_ref.value は tiptap_content_ref と同じ内容である
  //       A: 下記のように処理すると良い
  //            1) minieditorlangsrc="x=-b\pm \frac {\sqrt{b^2-4ac}}{2a}" を切り出す
  //            2) 切り出した minieditorlangsrc="x=-b\pm \frac {\sqrt{b^2-4ac}}{2a}" に対し、
  //               Webページでは数式を表すためにLaTeX表記が使えるMathJaxを利用します、
  //                http://www.ic.daito.ac.jp/~mizutani/html/mathexpress.html
  //                https://www.mathjax.org/
  //                  ※ IE で動作しない？
  // -------------------------------------------------------------------------------
  // <p data-uid="86f2b207-7489-4975-bb14-9e71541b4500">before z-math</p>
  // <z-math 
  ...
  //   </div>
  // </z-math>
  // <p data-uid="09491d91-1585-43d4-9da7-5abb9bfd184d">after z-math</p>
  // -------------------------------------------------------------------------------
  //= 

  //= <z-code> 子 本コンテンツを収集する
  // -------------------------------------------------------------------------------
  ...
  //= python-shell を呼出し、[ z-diag, z-math ] コンテンツを表示出来る [ z-diag ⇒ svg, z-math ⇒ LaTex ] に変換する
  //  [ in] tiptap ⇒ mii_content_ref.value （tiptap_content_ref.value も同じ）
  //  [ in] mode := visible | （保留）
  //  [out] modContent
  //  [out] hasLaTex  ※ 何らかの形で return し、Html header に MathJax（仮）を呼び出すコードを作成する
  //-
  const out_dc = await modCustomTagContent(mii_content_ref.value, "visible");
  let modContent = out_dc["modContent"];
  let hasLaTex   = out_dc["hasLaTex"];
  if (modContent == null) {
    console.log("Warning: modContent == null");
    modContent = mii_content_ref.value;
  }

  assets_ref.value["hasLaTex"] += hasLaTex;   // !! 初期化を忘れずに
...
defineExpose({
  // galaxy_ref,
  makeRawContentJson,   // used to assets (.css .html)
  saveDbContent,        // database i/o
  loadDbContent,        // database i/o
})
...
.meta-content-container {
  display: flex;            // 余白に合わせて伸張する 【備考】何故か grid は動作しなかった 2023/10/12
  flex-direction: column;   // 余白に合わせて伸張する

  width: 100%;
  height: 100%;

  //= 2024.01.04a 削除した
  // background-color: rgba(0, 0, 245, 1.0);
  // color: rgba(0, 0, 0, 1.0);
  // border: solid 4px rgba(0, 255, 0, 1.0);  
}
