<template>
  <div 
    ...
    title = "galaxy_ref :- EditSvgPolyline galaxy: div, Position, Size の基準（当 Html Frame の 辺界線）" 
  >
    ...
    <div
      ...
      title = "earth_ref :- EditSvgPolyline earth: div, canvas, to warp svg-root"
    >  
...
const svg_polyline_ref = inject('svg_polyline_ref');
...
const updateLPanelAttrs = () => { // 2024.01.06c L-attr に反映
  ...
  try {
    svg_polyline_ref.value = structuredClone(htmsvg_outer_ref.value); // 2024.01.06c L-attr に反映
...
watch(svg_polyline_ref, (newVal) => { // 2023.12.19a
  if (!((actingEditRef.value.com_type == props.com_type) && (actingEditRef.value.uuid == uuidRef.value))) {  // 2024.01.06b
  ...
  console.log(`$$$$ watch svg_polyline_ref: htmsvg_outer_ref.value updated to ${JSON.stringify(htmsvg_outer_ref.value, null, 2)}`);
}, { deep: true });
...
const makeRawContentJson = (rootContainerRequired) => {
  ...
  //= 子 本コンテンツを収集する
  let svg = `<polyline points="${htmsvg_outer_ref.value.points}" stroke="${htmsvg_outer_ref.value.stroke}" stroke-width="${htmsvg_outer_ref.value.stroke_width}" fill="${htmsvg_outer_ref.value.fill}"/>`;
