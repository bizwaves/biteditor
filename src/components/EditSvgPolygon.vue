<template>
  <div 
    ...
    title = "galaxy_ref :- EditSvgPolygon galaxy: div, Position, Size の基準（当 Html Frame の 辺界線）" 
  >
    ...
    <div
      ...
      title = "earth_ref :- EditSvgPolygon earth: div, canvas, to warp svg-root"
    >  
      ...
      -->
        <v-textarea 
            ...
            bg-color    = "black"
            color       = "yellow"
            v-model     = "htmsvg_outer_ref.points"
        >
        </v-textarea>
      </div>
      <!-- END Raw-Editor ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
      ...
      <div v-if = "true" class = "svg_outer">
        ...
        <svg v-if = "true" ref = "moon_ref" class = "svg_root" :viewBox = "viewBoxStr" >
          ...
          <g v-if = "true" >
            <title> EditRectangle 実要素 + Ctrl Group </title>

            <g v-if = "true">
              <title>EditRectangle 【実要素】Group </title>

              <!-- <polygon points="50,50 150,50 150,150, 50,150 125,75 150,150" stroke="black" stroke-width="1" fill="lightgreen"/> -->
              <polygon
                  ref = "htmsvg_dom_ref"
                  :points = "htmsvg_outer_ref.points" 
...
const svg_polygon_ref = inject('svg_polygon_ref');
...
const htmsvg_outer_ref = ref({           // 2023.11.15
...
// console.log(`~~~~~~~~~~~~~ htmsvg_outer_ref:` + JSON.stringify(htmsvg_outer_ref.value, null, 2));
...
const updateLPanelAttrs = () => { // 2024.01.06c L-attr に反映
  ...
  try {
    svg_polygon_ref.value = structuredClone(htmsvg_outer_ref.value); // 2024.01.06c L-attr に反映
...
watch(svg_polygon_ref, (newVal) => { // 2023.12.19a
  if (!((actingEditRef.value.com_type == props.com_type) && (actingEditRef.value.uuid == uuidRef.value))) {  // 2024.01.06b
  ...
  console.log(`$$$$ watch svg_polygon_ref: htmsvg_outer_ref.value updated to ${JSON.stringify(htmsvg_outer_ref.value, null, 2)}`);
}, { deep: true });
...
const makeRawContentJson = (rootContainerRequired) => {
  ...
  let svg = `<polygon points="${htmsvg_outer_ref.value.points}" stroke="${htmsvg_outer_ref.value.stroke}" stroke-width="${htmsvg_outer_ref.value.stroke_width}" fill="${htmsvg_outer_ref.value.fill}"/>`;
