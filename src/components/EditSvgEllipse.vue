<template>
  <div 
    ...
    title = "galaxy_ref :- EditSvgEllipse galaxy: div, Position, Size の基準（当 Html Frame の 辺界線）" 
  >
    ...
    >  
      ...
      <div v-if = "true" class = "svg_outer">
        ...
        <svg v-if = "true" ref = "moon_ref" class = "svg_root" :viewBox = "viewBoxStr" >
          ...
          <g v-if = "true" >
            <title> EditSvgEllipse 実要素 + Ctrl Group </title>

            <g v-if = "true">
              <title>EditSvgEllipse 【実要素】Group </title>
              <ellipse 
                  :cx = "htmsvg_outer_ref.cx" 
                  :cy = "htmsvg_outer_ref.cy" 
                  :rx = "htmsvg_outer_ref.rx" 
                  :ry = "htmsvg_outer_ref.ry" 
...
const svg_ellipse_ref = inject('svg_ellipse_ref');
...
const updateLPanelAttrs = () => { // 2024.01.06c L-attr に反映
  ...
  try {
    svg_ellipse_ref.value = structuredClone(htmsvg_outer_ref.value); // 2024.01.06c L-attr に反映
...
watch(c, (val, oldVal) => { // SvgEditor resizing_frame サイズが反映されたと確認出来た 2023/06/14
  ...
  htmsvg_outer_ref.value["rx"] = Math.floor(val.w / 2 - htmsvg_outer_ref.value["stroke_width"] / 2);
  htmsvg_outer_ref.value["ry"] = Math.floor(val.h / 2 - htmsvg_outer_ref.value["stroke_width"] / 2);
...
watch(actingEditEventRef, (val) => {
  ...
  if ((val.cate == 'attr') && (val.event == "svg_ellipse")) {
    //= 2023.12.19a watch へ移動した
    // htmsvg_outer_ref.value = val.val;

    // //= htmsvg_outer_ref.rx y を修正する
    // htmsvg_outer_ref.value["cx"] = Math.round(c.w / 2);  // !!
    // htmsvg_outer_ref.value["cy"] = Math.round(c.h / 2);  // !!
    // htmsvg_outer_ref.value["rx"] = Math.floor(c.w / 2 - htmsvg_outer_ref.value["stroke_width"] / 2);
    // htmsvg_outer_ref.value["ry"] = Math.floor(c.h / 2 - htmsvg_outer_ref.value["stroke_width"] / 2);
...
watch(svg_ellipse_ref, (newVal) => { // 2023.12.19a
  if (!((actingEditRef.value.com_type == props.com_type) && (actingEditRef.value.uuid == uuidRef.value))) {  // 2024.01.06b
  ...
  console.log(`$$$$ watch svg_ellipse_ref: htmsvg_outer_ref.value updated to ${JSON.stringify(htmsvg_outer_ref.value, null, 2)}`);
...
const makeRawContentJson = (rootContainerRequired) => {
  ...
  //= 子 本コンテンツを収集する
  let svg = "";
  svg += `<ellipse cx = "${htmsvg_outer_ref.value.cx}" cy = "${htmsvg_outer_ref.value.cy}" rx = "${htmsvg_outer_ref.value.rx}" ry = "${htmsvg_outer_ref.value.ry}" stroke = "${htmsvg_outer_ref.value.stroke}" stroke-width = "${htmsvg_outer_ref.value.stroke_width}" fill = "${htmsvg_outer_ref.value.fill}" />`
