<template>
  <div 
    ...
    title = "galaxy_ref :- EdiSvgLine galaxy: div, Position, Size の基準（当 Html Frame の 辺界線）" 
  >
    ...
    <div
      ...
      title = "earth_ref :- EditSvgLine earth: div, canvas, to warp svg-root"
    >  
      ...
      <v-container  
        ...
        title = "menu_container_ref :- EditSvgLine Menu Container"
      >
        <div v-if="false"
          ref="menu_area_ref" 
      ...
      <div v-if = "true" class = "svg_outer">
        ...
        <svg v-if = "true" ref = "moon_ref" class = "svg_root" :viewBox = "viewBoxStr" >
          <title>moon_ref :- svg of EditSvgLine</title> 

          <g v-if = "true" >
            <title> EditSvgLine 実要素 + Ctrl Group </title>

            <g v-if = "true">
              <title>EditSvgLine 【実要素】Group </title>
              ...
              <line
                  ref = "mii_htmsvg_ref"
                  class = "mii_svg" 
                  :x1="htmsvg_outer_ref.x1" :y1="htmsvg_outer_ref.y1" :x2="htmsvg_outer_ref.x2" :y2="htmsvg_outer_ref.y2" 
                  :stroke="htmsvg_outer_ref.stroke" :stroke-width="htmsvg_outer_ref.stroke_width" 
              >
...
const svg_line_ref = inject('svg_line_ref');
...
const updateLPanelAttrs = () => { // 2024.01.06c L-attr に反映
  ...
  try {
    svg_line_ref.value = structuredClone(htmsvg_outer_ref.value); // 2024.01.06c L-attr に反映
...
watch(c, (val, oldVal) => { // SvgEditor resizing_frame サイズが反映されたと確認出来た 2023/06/14
  ...
  htmsvg_outer_ref.value.x1 = remainder;
  htmsvg_outer_ref.value.y1 = (val.h >= 2 * remainder ? val.h - 2 * remainder : 240) / 2;
  htmsvg_outer_ref.value.x2 = val.w >= 2 * remainder ? val.w - 2 * remainder : 320;
  htmsvg_outer_ref.value.y2 = (val.h >= 2 * remainder ? val.h - 2 * remainder : 240) / 2;
  updateLPanelAttrs(); // 2024.01.06c L-attr に反映
});
...
watch(actingEditEventRef, (val) => {
  ...
  if ((val.cate == 'attr') && (val.event == "svg_line")) {
...
watch(svg_line_ref, (newVal) => { // 2023.12.19a
  if (!((actingEditRef.value.com_type == props.com_type) && (actingEditRef.value.uuid == uuidRef.value))) {  // 2024.01.06b
  ...
  console.log(`$$$$ watch svg_line_ref: htmsvg_outer_ref.value updated to ${JSON.stringify(htmsvg_outer_ref.value, null, 2)}`);
}, { deep: true });
...
const makeRawContentJson = (rootContainerRequired) => {
  ...
  //= 子 本コンテンツを収集する
  let b = htmsvg_outer_ref;
  let svg = "";
  svg += `<line x1="${b.value.x1}" y1="${b.value.y1}" x2="${b.value.x2}" y2="${b.value.y2}" stroke="${b.value.stroke}" stroke-width="${b.value.stroke_width}" />`
