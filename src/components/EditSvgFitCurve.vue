...
<template>
  <div 
    ...
    title = "galaxy_ref :- EditSvgFitCurve galaxy: div, Position, Size の基準（当 Html Frame の 辺界線）" 
  >
    ...
    <div
      ...
      title = "earth_ref :- EditSvgFitCurve earth: div, canvas, to warp svg-root"
    >  
      ...
      <v-container  
        ...
        title = "menu_container_ref :- EditSvgFitCurve Menu Container"
      >
        <div v-if="false"
          ref="menu_area_ref" 
      ...
      <div v-if = "true" class = "svg_outer">
        ...
        <svg v-if = "true" ref = "moon_ref" class = "svg_root" :viewBox = "viewBoxStr" >
          <title>moon_ref :- svg of EditSvgFitCurve</title> 

          <g v-if = "true" >
            <title> EditSvgFitCurve 実要素 + Ctrl Group </title>

            <g v-if = "true">
              <title>EditSvgFitCurve 【実要素】Group </title>
                ...
                <template v-if="true">
                  ...
                  <g>
                      ...
                      -->
                      <g v-for = "item in drawRef.path" :key = "item.id" >
                          <path 
                              :d = "item.path" 
                      ...
                      <template v-for = "item in drawRef.polyline" :key = "item.id" >
                        <g v-show="!isPreviewModeRef">
                            <polyline v-if="!(makeLineContent(item) == null)"  class = "Connect" :points = "makeLineContent(item)" />
                        </g>
                      </template>
                      ...
                      <template v-for = "item in drawRef.point" :key = "item.id">
                        <g v-show="!isPreviewModeRef">
                            <circle v-if="!(makeCtrlPointStyle(item) == null)" :class = "makeCtrlPointStyle(item)" :cx = "item.x" :cy = "item.y" :r = "CtrlPtsShape.CircleSize" />
                        </g>
                      </template>
                  </g>
        ...
        <div v-show="isDevModeRef" class="abled_ctrl_points_frame">
          <div class="rela_abled_ctrl_points_base">
            <template v-for = "item in drawRef.point" :key = "item.id">
              <div v-show = "!isPreviewModeRef"
                  :id = "item.id"
                  title = "Ctrl Point"
...
import { svgFitCurveMachine } from './svgFitCurveMachine';
...
const svg_fitcurve_ref = inject('svg_fitcurve_ref');
...
watch(c, (val, oldVal) => { // SvgEditor resizing_frame サイズが反映されたと確認出来た 2023/06/14
  console.log("$$$$EditSvgFitCurve watch galaxy c val: " + val.w + " - " + + val.h + " oldVal: " + oldVal.w + " - " + + oldVal.h);
...
console.log("@@@@@@@@ EditSvgFitCurve props: " + JSON.stringify(props));
...
watch(svg_fitcurve_ref, (newVal) => { // 2023.12.19a
  ...
  htmsvg_outer_ref.value.z_index = newVal["z_index"];

  familyZIndexRef.value = Number(newVal["z_index"]); // !!
  // console.log(`$$$$ watch svg_fitcurve_ref: htmsvg_outer_ref.value updated to ${JSON.stringify(htmsvg_outer_ref.value, null, 2)}`);
}, { deep: true });
...
const svgFitCurveService = interpret(svgFitCurveMachine);


//= a) free-draw input, fit-curve buffer 
const fsmDrawRef = ref({  // FSM 元データ // 2023.12.12a
...
const drawRef = ref({     // FSM 版 描画用データ
    path      : [],       // パス[],  例: [ { 'id' : 'foo', 'path' : "M 25 25 C 25 25 25 25 25 25" }
    point     : [],       // 点,      例: [ { "x": 25,  "y": 25, id : "b1", anchor : false, "auto" : false },
    polyline  : [],       // 制御線,  例: [ { points : [ { "x": 25,  "y": 25 }, { "x": 25,  "y": 25 } ], id : "ab" }
});


const makeDrawFromFsm = () => { // 2023.12.12a
  // console.log("makeDrawFromFsm");

  const curves = fsmDrawRef.value.curves;

  drawRef.value.point     = [];   // 初期化
  drawRef.value.polyline  = [];
  drawRef.value.path      = [];

  // console.log("  curves: " + JSON.stringify(curves, null, 2));

  for (const cu of curves) {
    const curve = (cu as any).curve;

    // console.log("  curve.bez_polyline: " + JSON.stringify(curve.bez_polyline, null, 2));
    // console.log("  state: " + curve.state);

    if (curve.state == 'fited') {
      (drawRef.value.path as any).push({ path: curve.path, id: curve.uuid });
      drawRef.value.point    = (drawRef.value.point    as any).concat(curve.bez_point);
      drawRef.value.polyline = (drawRef.value.polyline as any).concat(curve.bez_polyline);

      // console.log("  drawRef.value.polyline: " + JSON.stringify(drawRef.value.polyline, null, 2));
    } else {
      (drawRef.value.path as any).push({ path: curve.path, id: curve.uuid });
    }
  }

  // console.log("  FSM 版 描画用 path: "    + JSON.stringify(drawRef.value.path,     null, 2));
  // console.log("  FSM 版 描画用 polylne: " + JSON.stringify(drawRef.value.polyline, null, 2));
  // console.log("  FSM 版 描画用 point: "   + JSON.stringify(drawRef.value.point,    null, 2));
}
...
//= 
svgFitCurveService.onTransition((state) => {
  // console.log('*** onTransition event: ' + state.event.type + ' state: ' + state.value); // DEBUG
  ...
  // if CORE_DATA_LOADED event
  if (state.event.type == 'CORE_DATA_LOADED') {
    // console.log("$$$$ CORE_DATA_LOADED");
  }

  fsmDrawRef.value = state.context.fsmDrawRef.value;
...
const htmsvg_outer_ref = ref({           // 2024.01.08s 2023.11.15 !! read 時 write back to drawingStatRef etc.
  fsm_draw  : fsmDrawRef.value,          // 2024.01.08s
  draw      : drawRef.value,
...
watch(fsmDrawRef, (newVal) => { // 2024.01.08s
  htmsvg_outer_ref.value.fsm_draw = newVal;
  // console.log(`$$$$ watch fsmDrawRef: htmsvg_outer_ref.value updated to ${JSON.stringify(htmsvg_outer_ref.value, null, 2)}`);
}, { deep: true });


watch(drawRef, (newVal) => { // 2024.01.08s
...
const updateLPanelAttrs = () => { // 2024.01.06c L-attr に反映
  ...
  try {
    svg_fitcurve_ref.value = structuredClone(htmsvg_outer_ref.value); // 2024.01.06c L-attr に反映
...
function makeLineContent(poly) {
  // console.log("$$$$$ makeLineContent poly: " + JSON.stringify(poly));   // dict | 2
  let polyStr = null;
  try {
    polyStr = poly.points[0]["x"] + "," + poly.points[0]["y"] + " " + poly.points[1]["x"] + "," + poly.points[1]["y"];
  } catch (error) {
    // console.error(error);
  }
...
function makeCtrlPointStyle(po) {
  // console.log("$$$$$ makeCtrlPointStyle po: " + JSON.stringify(po));   // dict | 4
  let st = null;
  try {
    if ('anchor' in po) {
      st = 'EndPoint';
...
const setCtrlPtsAbleTgtRef = (el) => {
  ...
  //= 
  if (el && el.id) {  // メモ 2023.12.01 el.id が null になることがある
...
function onKeyDown(e) {
  ...
  if (e.shiftKey === true) {            // ZOOM; e.ctrlKey | e.shiftKey | e.altKey | e.metaKey
    ...
    switch (e.key) {
      case 'g':
        svgFitCurveService.send({ type: 'KEY_CODE_G' });   // g := （新しい文明の）アンカー
        break;
      case 'z':
        svgFitCurveService.send({ type: 'KEY_CODE_Z' });   // z := closepath
        break;
      case 'd':                                        // d := debug dump
        coreDump();
        break;
      case 'm':                                        // m := move
        svgFitCurveService.send({ type: 'KEY_CODE_M' });
        break;
      case 'l':                                        // l := line
        svgFitCurveService.send({ type: 'KEY_CODE_L' });
        break;
      case 'c':                                        // c := curve
        svgFitCurveService.send({ type: 'KEY_CODE_C' });
        break;
      case 's':                                        // s := smooth curve
        svgFitCurveService.send({ type: 'KEY_CODE_S' });
        break;
      case 'q':                                        // q := quadratic curve
        svgFitCurveService.send({ type: 'KEY_CODE_Q' });
        break;
      case 't':                                        // t := smooth quadratic curve
        svgFitCurveService.send({ type: 'KEY_CODE_T' });
        break;
      case 'a':                                        // a := elliptical arc
        svgFitCurveService.send({ type: 'KEY_CODE_A' });
        break;
      case 'h':                                        // h := horizontal line
        svgFitCurveService.send({ type: 'KEY_CODE_H' });
        break;
      case 'v':                                        // v := vertical line
        svgFitCurveService.send({ type: 'KEY_CODE_V' });
        break;
      case 'x':                                        // x := cancel
        svgFitCurveService.send({ type: 'KEY_CODE_X' });
        break;
      case 'r':                                        // r := reset
        svgFitCurveService.send({ type: 'KEY_CODE_R' });
        break;
      case 'p':                                        // p := pause
        svgFitCurveService.send({ type: 'KEY_CODE_P' });
        break;
      case 'o':                                        // o := resume
        svgFitCurveService.send({ type: 'KEY_CODE_O' });
        break;
      case 'n':                                        // n := next
        svgFitCurveService.send({ type: 'KEY_CODE_N' });
        break;
      case 'b':                                        // b := back
        svgFitCurveService.send({ type: 'KEY_CODE_B' });
        break;
      case 'f':                                        // f := forward
        svgFitCurveService.send({ type: 'KEY_CODE_F' });
        break;
      case 'e':                                        // e := end
        svgFitCurveService.send({ type: 'KEY_CODE_E' });
        break;
      case 'i':                                        // i := insert
        svgFitCurveService.send({ type: 'KEY_CODE_I' });
        break;
      case 'u':                                        // u := undo
        svgFitCurveService.send({ type: 'KEY_CODE_U' });
        break;
      case 'y':                                        // y := redo
        svgFitCurveService.send({ type: 'KEY_CODE_Y' });
        break;
      case 'w':                                        // w := write
        svgFitCurveService.send({ type: 'KEY_CODE_W' });
        break;
      case 'k':                                        // k := kill
        svgFitCurveService.send({ type: 'KEY_CODE_K' });
        break;
      case 'j':                                        // j := join
        svgFitCurveService.send({ type: 'KEY_CODE_J' });
        break;
      case '1':                                        // 1 := 1st
        svgFitCurveService.send({ type: 'KEY_CODE_1' });
        break;
      case '2':                                        // 2 := 2nd
        svgFitCurveService.send({ type: 'KEY_CODE_2' });
        break;
      case '3':                                        // 3 := 3rd
        svgFitCurveService.send({ type: 'KEY_CODE_3' });
        break;
...
async function sql_load_content(item, self_only = false) {
  ...
  const inner_obj = svg["inner"];

  //= 2024.01.08s お試し
  htmsvg_outer_ref.value = inner_obj;
  fsmDrawRef.value = inner_obj['fsm_draw'];
  drawRef.value = inner_obj['draw'];
  familyZIndexRef.value = Number(inner_obj["z_index"]);

  //= 2023.12.12 お試し
  //  htmsvg_outer_ref.value.draw = record["svg"];
  //   ※ class BezPoint, Bezier3, BezierFrame のインスタンスを作成し、データを設定し、本格的 fsm context を作成しなければなりません。
  //=
  nextTick(() => {                      // 2023.12.11f お試し
    svgFitCurveService.send({           // ev := 任意のイベント（dict） 
      type: 'CORE_DATA_LOADED',         // type := イベントの種類（str）
      core_data: inner_obj['fsm_draw'], // core_data := イベントの引数 1（dict）
      // ...                            // イベントの引数 2, 3, 4, ...（any）
    });
  })
...
const makeRawContentJson = (rootContainerRequired) => {
  ...
  // <g v-for = "item in drawRef.path" :key = "item.id" >
  for (const item of drawRef.value.path) {
    svg += `<path d = "${item.path}" stroke = "${htmsvg_outer_ref.value.stroke}" fill = "${htmsvg_outer_ref.value.fill}" stroke-width = "${htmsvg_outer_ref.value.stroke_width}" />\n`
  }
  svg += `</g>\n`
...
onMounted(async () => { // after DOM nodes be inserted
  await nextTick();
  console.log('$$$$$ Enter onMounted ' + props.com_type + ' ' + uuidRef.value);

  updateLPanelAttrs(); // 2024.01.06c L-attr に反映 

  // BEGIN START SVG PATH Fsm-SERVICEs ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  //= 外では動作しないので、ここへ移動した
  console.log('***** onMounted 1) Start svgPathService');
  svgFitCurveService.start();

  console.log('**** onMounted 2) send INIT to svgPathService');
  svgFitCurveService.send('INIT');

  console.log('*** onMounted 3) send UPDATE_RECT to svgPathService');
...
onBeforeUnmount(() => {   // before unmountafter unmounted? Vue 2.x beforeDestroy
  ...
  svgFitCurveService.stop();     // ここで、FsmService を停止する(これがないと、FsmService が二重起動する)
