<template>
  <div 
    ...
    title = "galaxy_ref :- EditSvgRect galaxy: div, Position, Size の基準（当 Html Frame の 辺界線）" 
  >
    ...
    <div
      ...
      title = "earth_ref :- EditSvgRect earth: div, canvas, to warp svg-root"
    >  
      ...
      <div v-if = "true" class = "svg_outer">
        ...
        <svg v-if = "true" ref = "moon_ref" class = "svg_root" :viewBox = "viewBoxStr" >
          <title>moon_ref :- svg of EditSvgRect</title> 

          <g v-if = "true" >
            <title> EditSvgRect 実要素 + Ctrl Group </title>

            <g v-if = "true">
              <title>EditSvgRect 【実要素】Group </title>

              <!-- TODO :style = "htmsvg_outer_ref.style"  -->
              <rect ref = "mii_htmsvg_ref"
                  class = "mii_svg" 
                  :x = "htmsvg_outer_ref.x" 
                  :y = "htmsvg_outer_ref.y" 
                  :width = "htmsvg_outer_ref.w" 
                  :height = "htmsvg_outer_ref.h" 
                  :fill = "htmsvg_outer_ref.fill"
                  :stroke = "htmsvg_outer_ref.stroke"
                  :stroke-width = "htmsvg_outer_ref.stroke_width"
              >
                  <title> EditRect 本コンテンツ </title>
              </rect>
            </g>  
          </g> 
        </svg> 
...
const svg_rect_ref = inject('svg_rect_ref');
...
const selColorsRef: any = inject('selColorsRef');               // 2023.12.16a color picker info, 参考 htmsvg_outer_ref の stroke fill
...
const htmsvg_outer_ref = ref({           // !! 後に watch(c に更新される 2023.11.15
  ...
  stroke_width: 1,                    // 2023.12.19f SvgEditor svg_rect_ref より 足りない⇒追加した
...
const updateLPanelAttrs = () => { // 2024.01.06c L-attr に反映
  ...
  try {
    svg_rect_ref.value = structuredClone(htmsvg_outer_ref.value); // 2024.01.06c L-attr に反映
...
watch(actingEditRef, (val) => { //  2023.12.16a
  ...
  nextTick(() => {
    ...
    updateLPanelAttrs(); // !! L-attr に反映, 2024.01.06c
  })
}, { deep: true });
...
watch(c, (val, oldVal) => { // SvgEditor resizing_frame サイズが反映されたと確認出来た 2023/06/14
  ...
  htmsvg_outer_ref.value.x = remainder;
  htmsvg_outer_ref.value.y = remainder;
  htmsvg_outer_ref.value.w = val.w >= 2 * remainder ? val.w - 2 * remainder : 320;
  htmsvg_outer_ref.value.h = val.h >= 2 * remainder ? val.h - 2 * remainder : 240;
...
const props = defineProps({ // I/F: 親⇒子のデータの受け渡し
  com_type: { // edit_svg | edit_html_tag | ..
    type: String,
    default: "edit_svg_rect",
  },

  // uuid: { // 【廃止】2023.11.18 nodel familyIdRef へ移行; uuid は、このコンポーネントの一意の識別子です
  //   type: String,
  //   default: uuidv4(),
  // },
...
const onMeClick = (e: any) => {
  ...
  console.assert(props.com_type != null, "props.com_type is null");         // 2023.11.19z
  console.assert(uuidRef.value != null, "uuidRef.value is null");           // ..
  console.assert(familyIdRef.value != null, "familyIdRef.value is null");   // ..
...
watch(actingEditEventRef, (val) => {
  ...
  console.log(`  val: ${JSON.stringify(val, null, 2)}`); // val := com_type uuid cate event

  // 2) cate attr:  attr := = SvgEditor.svg_rect_ref, event := updated | _
  if ((val.cate == 'attr') && (val.event == "updated")) {  // !!
    //= 調査: この現象は、Vue.jsのリアクティブシステムの動作に関連している可能性があります
    // 【現象】
    //    SvgEditor.vueでz_indexが 50 から 100に変更されたとき、val.val["z_index"]（新しいz_indexの値）とhtmsvg_outer_ref.value.z_index（古いz_indexの値）が
    //    両方とも100であることがログから確認できます。
    // 【解析】
    //    a) この現象は、Vue.jsのリアクティブシステムの動作に関連している可能性があります。svg_rect_ref.value = htmsvg_outer_ref.value; というコードは、
    //      svg_rect_refとhtmsvg_outer_refが同じオブジェクトを参照するように設定します。そのため、一方のRefのプロパティを変更すると、他方のRefの同じプロパティも自動的に変更されます。
    //    b) しかし、htmsvg_outer_ref.value = val.val; というコードをコメントアウトした場合、htmsvg_outer_refは新しいオブジェクトを参照するように設定されなくなります。
    //      その結果、svg_rect_refのプロパティを変更しても、htmsvg_outer_refの同じプロパティは自動的には変更されません。
    //    c) この問題を解決するためには、svg_rect_refのプロパティが変更されたときにhtmsvg_outer_refの同じプロパティを手動で更新するコードを追加する必要があります。
    // 【対策】
    //    watch(svg_rect_ref, (newVal) => { htmsvg_outer_ref.value = newVal; }); というコードを追加しました(2023.12.19a)。
    //=
    // console.log('  1) val.val: ' + JSON.stringify(val.val));                                              // val.val := { "z_index": 100 }    
    // console.log('  2) htmsvg_outer_ref.value: ' + JSON.stringify(htmsvg_outer_ref.value));                // htmsvg_outer_ref.value := { "z_index": 100 }
    // console.log(`  3) z_index from ${htmsvg_outer_ref.value.z_index} changed to ${val.val["z_index"]}?`); // z_index from 100 changed to 100 ?

    return; // 2023.12.19a へ移行したため、ここで処理なし
  }
...
watch(svg_rect_ref, (newVal) => { // 2023.12.19a
  if (!((actingEditRef.value.com_type == props.com_type) && (actingEditRef.value.uuid == uuidRef.value))) {  // 2024.01.06b
  ...
  console.log(`$$$$ watch svg_rect_ref: htmsvg_outer_ref.value updated to ${JSON.stringify(htmsvg_outer_ref.value, null, 2)}`);
}, { deep: true });
...
const makeRawContentJson = (rootContainerRequired) => {
  ...
  let svg = `<rect x = "${htmsvg_outer_ref.value.x}" y = "${htmsvg_outer_ref.value.y}" width = "${htmsvg_outer_ref.value.w}" height = "${htmsvg_outer_ref.value.h}" ` +
    `style = "fill:${htmsvg_outer_ref.value.fill};stroke-width:${htmsvg_outer_ref.value.stroke_width};stroke:${htmsvg_outer_ref.value.stroke}" />`
...
onUnmounted(() => {   // after unmounted
  console.log(props.com_type + "::" + uuidRef.value + " Unmounted!");
})


onBeforeUpdate(() => {  // 2023.12.13b TODO ↓必要か調査
  //=
