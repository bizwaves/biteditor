<template>
  <div 
    ...
    title = "galaxy_ref :- EditSvgSrc galaxy: div, Position, Size の基準（当 Html Frame の 辺界線）" 
  >
    ...
    <div
      ...
      title = "earth_ref :- EditSvgSrc earth: div, canvas, to warp svg-root"
    >
      ...
      <div v-if = "true" class = "svg_outer">
        ...
        <svg v-if = "true" ref = "moon_ref" class = "svg_root" :viewBox = "viewBoxStr" >
          <title>moon_ref :- svg of EditSvgSrc</title> 

          <g v-if = "true" >
            <title> EditSvgSrc 実要素 + Ctrl Group </title>

            <g v-if = "true">
              <title>EditSvgSrc 【実要素】Group </title>
...
const svg_src_ref = inject('svg_src_ref');
...
const htmsvg_container_ref = ref(null);  // DOM

const htmsvg_inner_ref = ref('<circle cx="100" cy="40" r="39" stroke="#f00" fill="#f88" />');

let madeInHis = false; // 2024.01.01a History から作成される時 true にする（History に追加されることを防ぐため） ※ 内容が変更された時、false にすることを忘れずに

const htmsvg_outer_ref = ref({
  draw: htmsvg_inner_ref.value,         // 2024.01.08s DB Fix Save/Load ↓ .value にした ※ あくまでも初期値であって、watch で更新されることを期待している
  ...
  color: appCfgRef.value.front_color,     // 2023.12.16a
...
const updateLPanelAttrs = () => { // 2024.01.06c L-attr に反映
  ...
  try {
    svg_src_ref.value = structuredClone(htmsvg_outer_ref.value); // 2024.01.06c L-attr に反映
...
watch(svg_src_ref, (newVal) => { // 2023.12.19a
  if (!((actingEditRef.value.com_type == props.com_type) && (actingEditRef.value.uuid == uuidRef.value))) {  // 2024.01.06b
  ...
  console.log(`$$$$ watch svg_src_ref: htmsvg_outer_ref.value updated to ${JSON.stringify(htmsvg_outer_ref.value, null, 2)}`);
}, { deep: true });
...
watch(htmsvg_inner_ref, (newVal) => { // 2024.01.08s
  htmsvg_outer_ref.value.draw = newVal;             // 2024.01.08s DB, History Etc.
  if (htmsvg_container_ref.value) {
    htmsvg_container_ref.value.innerHTML = newVal;  // 2024.01.08s 画面に反映
  }
...
const makeRawContentJson = (rootContainerRequired) => {
  ...
  //= 子 本コンテンツを収集する
  let svg = htmsvg_inner_ref.value;
