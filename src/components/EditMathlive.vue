...
import MiniMathliveEditor from './MiniMathliveEditor.vue'
...
const laTeX = ref(floatMinieditorRef.value.src != "" ? floatMinieditorRef.value.src : "f(x)=" );   // !! 【注意】fmini_content_ref は親に渡されていない
...
const cmenuItems = ref([              // cmenu 名前になった経緯: context menu(right click) ⇒ click menu TODO menu: nested-menu
  { id: 'who_am_i',           visible: true,    name: 'EditCodeMirror ' + uuidRef.value },
...
const onCmenuItemClick = (cmd: any) => {
  ...
  switch (cmd.id) {
    ...
    case 'cancel':
      ...
      break;
    case 'save':
      floatMinieditorRef.value.src = laTeX.value;   // ソースコードは、
      fmini_content_ref.value = laTeX.value;   // 両方は同じです
