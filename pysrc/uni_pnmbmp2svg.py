...
if __name__ == '__main__':
    ...
    # (Pdb) pp payload
    # {'cmd': 'potrace',
    # 'options': {'bin_file': 'potrace.exe',
    #             'out_stem': 'k1_drawing_potrace',
    #             'sys_path': 'C:\\homelocal\\bin\\potrace'}}
    ...
    if path.is_file():
        ...
        # C:\home\local\bin\potrace> .\potrace.exe -b svg C:\DOCs\SvgEditor\k1_drawing.bmp -o  C:\DOCs\SvgEditor\k1_drawing_potrace.svg
        ...
        cmd = f"{exe_name} -b svg {path} -o {dst_path}"
        result = do_cmd(cmd)
        response['error'] = list(result)
        response['exit_code'] = 0 if len(response['error']) == 0 else 1 
    else:
