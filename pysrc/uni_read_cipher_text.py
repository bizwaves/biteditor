...
from txt_crypter import TxtCrypter
...
if __name__ == '__main__':
    ...
    passPhrase = payload['sec_pass'] if 'sec_pass' in payload else "password123"
    encryptedData = payload['encrypted']
    
    response = {}

  
    #= READ encryptedData.encrypted
    # 	不要 when encryptedData.encrypted is not None
    #=
    ...
    #= 復号
    crypter = TxtCrypter()
    decrypted = crypter.decrypt(encryptedData, passPhrase)
    base64string = base64.b64encode(decrypted.encode()).decode()
    # base64string = base64.b64encode(decrypted.encode('shift_jis')).decode()
    response = { 'decrypted' : base64string }
